...
import { useEffect, useRef, useState } from 'react';
import { ChevronLeft, ChevronRight } from 'lucide-react';
import { getSliderImages } from '@/utils/ourWorksSlides';
import { SliderImage } from '@/types/ourWorksSlides';
...
const next = useCallback(() => {
 ...
 useEffect(() => {
  ...
  const getImageUrl = (index: number) =>
    ...
    (images[index]?.image?.formats?.large?.url || images[index]?.image?.url);
  ...
  return (
    <div className="relative w-full max-w-9xl overflow-hidden py-10 bg-white">
      ...
      >
        {/* Left Peek Image */}
        <div className="absolute left-0 ml-0 w-[180px] h-[350px] overflow-hidden z-10 rounded-xl shadow opacity-20">
          <Image
            src={getImageUrl((currentIndex - 1 + images.length) % images.length)}
            alt="Previous"
            fill
            className="object-cover object-right"
          />
        ...
        {/* Right Peek Image */}
        <div className="absolute right-0 mr-0 w-[180px] h-[350px] overflow-hidden z-10 rounded-xl shadow opacity-20">
          <Image
            src={getImageUrl((currentIndex + 2) % images.length)}
            alt="Next"
            fill
            className="object-cover object-left"
          />
